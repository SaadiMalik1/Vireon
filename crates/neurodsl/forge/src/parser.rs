...
pub fn parse(tokens: Vec<Token>) -> Result<Ast, ForgeError> {
    ...
    while let Some(token) = iter.next() {
        match token {
                ...
                if let Some(Token::Number(val)) = iter.next() {
                    if val > 255 {
                        return Err(ForgeError::ParserError(format!(
                            "SET_AMP value {} exceeds 8-bit limit",
                            val
                        )));
                    }
                    ...
                    return Err(ForgeError::ParserError(
                        "Expected number after SET_AMP".into(),
                    ));
                }
            }
            Token::SetFreq => {
                if let Some(Token::Number(val)) = iter.next() {
                    ...
                    return Err(ForgeError::ParserError(
                        "Expected number after SET_FREQ".into(),
                    ));
                }
            }
            Token::Wait => {
            ...
            Token::Shape => {
                if let Some(Token::Identifier(shape_name)) = iter.next() {
                    ...
                    if let Some(Token::Number(size)) = iter.next() {
                        if size > 255 {
                            return Err(ForgeError::ParserError(format!(
                                "SHAPE size {} exceeds 8-bit limit",
                                size
                            )));
                        }
                        ...
                        return Err(ForgeError::ParserError(
                            "Expected size number after shape name".into(),
                        ));
                    }
                } else {
                    return Err(ForgeError::ParserError(
                        "Expected shape name after SHAPE".into(),
                    ));
                }
            }
            Token::End => {
            ...
            Token::ReadSensor => {
                if let (Some(Token::Number(sensor)), Some(Token::Number(addr))) =
                    (iter.next(), iter.next())
                {
                    if sensor > 255 || addr > 255 {
                        return Err(ForgeError::ParserError(
                            "READ_SENSOR arguments must fit in 8 bits".into(),
                        ));
                    }
                    statements.push(Statement::ReadSensor(sensor as u8, addr as u8));
                } else {
                    return Err(ForgeError::ParserError(
                        "Expected sensor and address after READ_SENSOR".into(),
                    ));
                }
            }
            Token::LoopStart => {
                if let Some(Token::Number(iters)) = iter.next() {
                    if iters > 255 {
                        return Err(ForgeError::ParserError(format!(
                            "LOOP_START iterations {} exceeds 8-bit limit",
                            iters
                        )));
                    }
                    ...
                    return Err(ForgeError::ParserError(
                        "Expected iterations after LOOP_START".into(),
                    ));
                }
            }
            Token::LoopEnd => {
            ...
            Token::JumpIf => {
                if let (
                    Some(Token::Number(addr)),
                    Some(Token::Number(val)),
                    Some(Token::Number(target)),
